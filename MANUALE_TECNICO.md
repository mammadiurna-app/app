...
Uniche eccezioni: `xlsx-js-style.min.js` (libreria SheetJS con supporto stili, licenza Apache 2.0) e `jspdf.umd.min.js` (libreria jsPDF, licenza MIT), vendorizzate come file separati e caricate con `<script src="...">` prima dello script principale, usate rispettivamente da `exportXLS()` per generare file `.xlsx` con celle unite/centrate/bordate e da `exportPDF()` per generare il PDF. Scelta deliberata di non usare CDN esterni, per non introdurre una dipendenza di rete quando l'app viene usata con connessione scarsa (vedi commit che ha introdotto `xlsx-js-style.min.js` per il ragionamento completo, valido allo stesso modo per `jspdf.umd.min.js`). `jspdf.umd.min.js` espone la variabile globale `jspdf` (minuscolo), con `jspdf.jsPDF` come costruttore.
...
`buildAttendanceGrid(from,to)` è la funzione condivisa da `exportXLS()`, `exportPDF()` e `exportPNG()`: calcola l'elenco date (tutti i giorni del periodo, anche quelli senza presenze — iterando il calendario da `from` a `to` inclusi, non filtrando le chiavi di `allLogs`), raggruppa le timbrature per data/bambino (`byDateChild`), calcola quante colonne entrata/uscita servono per ciascuna data in base al numero massimo di turni che un bambino qualsiasi ha avuto quel giorno (`sessioniPerData`/`blockWidths` — 2 colonne = 1 turno, 4 = 2 turni, ecc., calcolato singolarmente per ogni data, non un valore fisso uguale per tutte), e la lista bambini coinvolti ordinata alfabeticamente. Ritorna `null` (con un toast d'errore) se il periodo non è valido o non ci sono presenze con orari.

`exportXLS()` usa `buildAttendanceGrid` per popolare un foglio `xlsx-js-style.min.js` (celle unite/centrate/bordate), serializzato con `XLSX.write(wb,{bookType:'xlsx',type:'array'})` in un `Blob`.

`exportPDF()` (bottone attivo in UI, alternativa all'XLS) usa la stessa `buildAttendanceGrid` per disegnare la tabella identica con l'API di disegno di jsPDF (`doc.rect(...)` per i bordi, `doc.text(...,{align:'center'})` per il testo centrato orizzontalmente — jsPDF non ha un vero "center verticale", si compensa con un offset fisso `+3` sul baseline). Dimensioni di pagina calcolate su misura in base al contenuto (`format:[width,height]` invece di un formato fisso tipo A4), con margine `MARGIN=14pt`. **Attenzione**: `new jspdf.jsPDF({format:[w,h], ...})` di default assume orientamento `portrait` e **scambia larghezza/altezza se `w > h`** a meno di passare esplicitamente `orientation:'l'` — bug scoperto in fase di test (la tabella veniva tagliata perché disegnata per una pagina larga ma la pagina effettiva risultava stretta e alta). Va sempre passato `orientation: width>=height?'l':'p'` insieme a un `format` custom.

`exportPNG()` — **non collegata a nessun bottone**, tenuta in archivio nel codice su richiesta esplicita per un eventuale uso futuro (es. se anche il PDF dovesse rivelarsi problematico su qualche dispositivo) — usa la stessa `buildAttendanceGrid` per disegnare la tabella su un `<canvas>` (bordi con `strokeRect`, testo centrato con `fillText`), poi esporta con `canvas.toBlob(...,'image/png')`.

Tutte e tre le funzioni condividono `condividiOScarica(blob,filename,shareFlagKey,label)`: `async`, tenta la **Web Share API** (`navigator.canShare({files:[file]})` + `navigator.share(...)`) per aprire il pannello di condivisione nativo (email, WhatsApp, AirDrop, ecc.), con fallback al download via `<a download>` se l'API non è supportata o se `canShare` rifiuta il file. Se l'utente annulla dal pannello nativo (`AbortError`), la funzione si ferma senza scaricare nulla. `shareFlagKey` è il nome della chiave `localStorage` (`xls_share_unsupported`, `pdf_share_unsupported`, `png_share_unsupported` — **separate per formato**: non è detto che si comportino allo stesso modo sullo stesso dispositivo, vedi sotto) usata per ricordare che la condivisione non funziona su questo dispositivo/browser e saltare il tentativo ai successivi export di quel formato.

Riscontrato in test reale (Android 10, Chrome 151, sia da PWA installata sia da scheda Chrome normale): `navigator.share()` con un file **xlsx** può fallire con `NotAllowedError: Permission denied` nonostante `canShare()` ritorni `true` e `navigator.userActivation.isActive`/`hasBeenActive` siano entrambi `true` — quindi non un problema di gesture utente, e non specifico alla modalità standalone/PWA (stesso errore identico da scheda Chrome normale). La condivisione nativa di file da altre app (es. Galleria) funziona regolarmente sullo stesso dispositivo, così come `navigator.share()` con un file **PNG** di test e con un file **PDF** di test — quindi non è una restrizione di sistema generale né un problema dell'API in sé, ma sembra un limite/bug specifico dell'implementazione Web Share per file non-immagine di quella build di Chrome/dispositivo (il PDF, pur essendo un "documento" come l'xlsx, condivide senza problemi — quindi il limite non è "documento vs immagine" in generale, ma sembra specifico al formato/MIME-type xlsx su quel dispositivo). Da qui la scelta di offrire `exportPDF()` come alternativa attiva quando l'export XLS non è condivisibile — preferito al PNG perché resta un vero documento (leggibile/stampabile) invece di una semplice immagine. Il manuale utente suggerisce inoltre, per l'XLS, il pulsante "Condividi" nella notifica di download di Chrome (passa per l'Android `DownloadManager` anziché per l'API `navigator.share`, e in quel caso funziona).
