...
`exportXLS()` è `async`: il workbook viene serializzato con `XLSX.write(wb,{bookType:'xlsx',type:'array'})` (non più `XLSX.writeFile`, che forza subito il download) in un `Blob`, poi si tenta la **Web Share API** (`navigator.canShare({files:[file]})` + `navigator.share(...)`) per aprire il pannello di condivisione nativo del sistema operativo (email, WhatsApp, AirDrop, ecc.), con fallback al download via link `<a download>` se l'API non è supportata (desktop, browser datati) o se `canShare` rifiuta il file. Se l'utente annulla la condivisione dal pannello nativo (`AbortError`), la funzione si ferma senza scaricare nulla, per non forzare comunque un salvataggio locale indesiderato.

Riscontrato in test reale (Android 10, Chrome 151, sia da PWA installata sia da scheda Chrome normale): `navigator.share()` può fallire con `NotAllowedError: Permission denied` nonostante `canShare()` ritorni `true` e `navigator.userActivation.isActive`/`hasBeenActive` siano entrambi `true` — quindi non un problema di gesture utente, e non specifico alla modalità standalone/PWA (stesso errore identico da scheda Chrome normale). La condivisione nativa di file da altre app (es. Galleria) funziona regolarmente sullo stesso dispositivo, quindi non è una restrizione di sistema generale — sembra un limite/bug della sola implementazione Web Share-per-file di quella build di Chrome/dispositivo. Se `share()` fallisce con un errore diverso da `AbortError`, il flag `xls_share_unsupported` viene salvato in `localStorage` per saltare il tentativo di condivisione ai successivi export su quel dispositivo (va dritto al download, evitando il giro a vuoto). Il manuale utente suggerisce, come alternativa, il pulsante "Condividi" nella notifica di download di Chrome (percorso diverso, passa per l'Android `DownloadManager` anziché per l'API `navigator.share`, e in questo caso funziona).
